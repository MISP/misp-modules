...
mispattributes = {
    ...
    "output": ["text", "sha1", "sha256", "md5", "link"],
}
moduleinfo = {
    "version": "0.3",
    ...
    "features": (
        ...
        " contained within the attribute in then enriched with data from VMRay mapped into MISP attributes."
    ),
        ...
        try:
            ...
            if sample:
                ...
                r["results"].append(
                    {
                        "types": "text",
                        "values": "VMRay Sample ID: %s" % sample["sample_id"],
                ...
                if include_vmrayjobids and len(jobs) > 0:
                    for job in jobs:
                        ...
                        r["results"].append(
                            {
                                "types": "text",
                                "values": "VMRay Job ID %s (%s - %s)" % (job_id, job_vm_name, job_configuration_name),
                            }
                        )
                return r
            else:
