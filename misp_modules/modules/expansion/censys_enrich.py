...
def handler(q=False):
    ...
    for t in types:
        try:
            ...
            # ip, ip-src or ip-dst
            if t[:2] == "ip":
                r = CensysHosts(api_id, api_secret).view(val)
                results.append(parse_response(r, attribute))
                found = True
            elif t == 'domain' or t == "hostname":
                #get ips
                endpoint = CensysHosts(api_id, api_secret)
                for r_list in endpoint.search(query=val, per_page=15, pages=1):
                    for r in r_list:
                        results.append(parse_response(r, attribute))
                        found = True
            elif 'x509-fingerprint' in t:
                # TODO switch to api_v2 once available
                # sill use api_v1 as Certificates endpoint in api_v2 doesn't yet provide all the details
                r = CensysCertificates(api_id, api_secret).view(val)
                results.append(parse_response(r, attribute))
                found = True
        except CensysException as e:
                misperrors['error'] = print("ERROR: param {} / resposne: {}".format(value, e))
                return misperrors
