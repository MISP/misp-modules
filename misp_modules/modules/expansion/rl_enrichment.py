...
def create_misp_object(
    ...
    for attr_def in attributes:
        ...
        known_misp_types = {
            ...
            'filename', 'size-in-bytes', 'text', 'link', 'datetime', 'domain',
            'ip-src', 'ip-dst', 'url', 'hostname', 'port', 'comment', 'counter',
            'boolean', 'float', 'hex', 'mime-type', 'entropy'
        }
    ...
    for attr in obj_dict.get("Attribute", []):
        if "disable_correlation" in attr:
            attr["disable_correlation"] = bool(attr["disable_correlation"])  # Keep as boolean like v0
        if "to_ids" in attr:
    ...
    *,
    ...
    if created_children >= limit:
        if not truncated_tag_emitted:
            try:
                results["Tag"].append(create_tag("rl", "note", f"{note_label} truncated after {limit} items"))
            except Exception:
                pass
    ...
    if det_uuid:
        ...
        existing_obj = next((o for o in results.get("Object", []) if o.get("uuid") == det_uuid), None)
        if existing_obj:
            # Merge attributes with dedup
            existing_attrs = existing_obj.get("Attribute", [])
            seen_pairs = {(str(a.get("object_relation", "")).lower(), str(a.get("value"))) for a in existing_attrs}
...
def _fetch_with_endpoint_fallback(
    ...
    for template in endpoints:
        ...
        try:
            ...
            method = type_config.get("request_method", "GET")
...
def handler(q: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not attr_value:
        for hash_key in ("sha256", "sha1", "md5", "sha512", "domain", "ip", "ip-src", "ip-dst", "url", "hostname"):
...
def _fetch_object_endpoint(
    ...
    try:
        ...
        if isinstance(parent_data, dict):
            ...
            if isinstance(sample, dict):
                for key, val in sample.items():
                    if isinstance(val, (str, int, float)) and val:
                        subs[key] = str(val)
                subs["hash"] = sample.get("sha256") or sample.get("sha1") or sample.get("md5") or subs["hash"]
        ...
        response = session.get(full_url, headers=headers, verify=verify_ssl, timeout=effective_timeout)
...
def _process_object_recursive(
    ...
    if not rels and obj_name in OBJECT_RELATIONSHIPS:
        ...
        rels = [{"type": rel_type, "targets": [target]} for target, rel_type in default_rels.items()]

    # Build attributes for this object
    attributes = _build_attributes_from_def(data, attr_def, original_value, results, api_context=api_context)
    ...
    if attributes:
        ...
        obj_comment = _resolve_dsl_in_comment(raw_obj_comment, data, api_context) if raw_obj_comment else obj_comment
        ...
        det_uuid = _compute_deterministic_uuid(obj_type, attributes, original_value, api_context.get("ioc_type") if api_context else None)
        if det_uuid:
    ...
    for child_key, child_def in nested_objects:
                ...
                if handler_match:
                    ...
                    if handler_name == "iterate_dns":
                                ...
                                for record in records:
                                    ...
                                    if record_type == "TXT":
                                        ...
                                        for ptype, pval in spf_endpoints:
                                            ...
                                            attrs = []
                                            if ptype == "ip":
                                                attrs.append(create_misp_attribute(misp_type="ip", value=pval, object_relation="ip"))
                                            elif ptype == "domain":
                                                attrs.append(create_misp_attribute(misp_type="hostname", value=pval, object_relation="hostname"))

                                            created_children, truncated_tag_emitted, child_obj = _add_child_object_with_limit(
                                                ...
                                                ioc_type=api_context.get("ioc_type") if api_context else None,
                                            )
                                        continue
                                    ...
                                    if record_type in ("A", "AAAA"):
                                        attrs.append(create_misp_attribute(misp_type="ip", value=record_value, object_relation="ip"))
                                    elif record_type in ("NS", "CNAME", "MX", "PTR"):
                                        attrs.append(create_misp_attribute(misp_type="hostname", value=record_value, object_relation="hostname"))
                        ...
                        if len(parts) >= 1:
                            ...
                            types = []
                            if len(parts) > 1:
                                types = [t.strip().lower() for t in parts[1].split(",") if t.strip()]
                            if not types:
                                types = ["ip", "domain"]
                                ...
                                if "ip" in types:
                                    for m in ipv4_re.finditer(val):
                                        ...
                                        if created_children >= MAX_DNS_CHILDREN:
                                            if not truncated_tag_emitted:
                                                try:
                                                    results["Tag"].append(create_tag("rl", "note", f"dns children truncated after {MAX_DNS_CHILDREN} items"))
                                                except Exception:
                                                    pass
                                                truncated_tag_emitted = True
                                            continue
                                        attrs = [create_misp_attribute(misp_type="ip", value=ioc, object_relation="ip")]
                                        child_obj = create_misp_object(child_obj_name, attrs)
                                        det_uuid = _compute_deterministic_uuid(child_obj_name, attrs, original_value, api_context.get("ioc_type") if api_context else None)
                                        if det_uuid:
                                            child_obj["uuid"] = det_uuid
                                            existing_obj = next((o for o in results.get("Object", []) if o.get("uuid") == det_uuid), None)
                                            if existing_obj:
                                    ...
                                    for m in host_re.finditer(val):
                                        ...
                                        if created_children >= MAX_DNS_CHILDREN:
                                            if not truncated_tag_emitted:
                                                ...
                                                truncated_tag_emitted = True
                                            continue
                                        attrs = [create_misp_attribute(misp_type="hostname", value=ioc, object_relation="hostname")]
                                        child_obj = create_misp_object(child_obj_name, attrs)
                                        det_uuid = _compute_deterministic_uuid(child_obj_name, attrs, original_value, api_context.get("ioc_type") if api_context else None)
                                        if det_uuid:
                                            child_obj["uuid"] = det_uuid
        ...
        if re.match(r'^obj:fetch(\[\d+\])?$', child_key) and isinstance(child_def, dict) and child_def.get("obj:uri"):
            if api_context:
                ...
                if endpoint_error:
                    try:
                        ...
                        results["Tag"].append(create_tag("rl", "fetch-error", f"{short_uri}: {endpoint_error}"))
                    except Exception:
                        pass
                    ...
                    if obj_template_key and obj_template_def:
                        ...
                        if isinstance(items, list):
                            ...
                            for item in items:
                                ...
                                dedup_key = item.get("sha256") or item.get("sha1") or item.get("md5")
                                if not dedup_key:
                                    # Check nested sample object (common in A1000 responses)
                                    sample = item.get("sample", {})
                                    if isinstance(sample, dict):
                                        dedup_key = sample.get("sha256") or sample.get("sha1") or sample.get("md5")
                                if not dedup_key:
                                ...
                                if created_children >= MAX_DNS_CHILDREN:
                                    if not truncated_tag_emitted:
                                        try:
                                            ...
                                            results["Tag"].append(create_tag("rl", "note", f"{obj_type} children truncated after {MAX_DNS_CHILDREN} items"))
                                        except Exception:
                                            pass
                                        truncated_tag_emitted = True
                                    continue
            ...
            if path_data is not None:
                ...
                if isinstance(path_data, list):
                    ...
                    for item in path_data:
                        ...
                        effective_limit = foreach_limit if foreach_limit != MAX_FOREACH_ITERATIONS else MAX_DNS_CHILDREN
                        if created_children >= effective_limit:
                            if not truncated_tag_emitted:
                                try:
                                    obj_type = child_def.get("obj:type", foreach_path)
                                    results["Tag"].append(create_tag("rl", "note", f"{obj_type} children truncated after {effective_limit} items"))
                                except Exception:
                                    pass
                                truncated_tag_emitted = True
                            continue
                        ...
                        if created_children >= effective_limit:
                            if not truncated_tag_emitted:
                                try:
                                    ...
                                    results["Tag"].append(create_tag("rl", "note", f"{obj_type} truncated after {effective_limit} items"))
                                except Exception:
                                    pass
                                truncated_tag_emitted = True
                            continue
        ...
        for pending in results["_pending_refs"]:
            ...
            if target_name in created_objects:
                ...
                for target_obj in target_objs:
                    ...
                    existing = {(r.get("referenced_uuid"), r.get("relationship_type")) 
...
def _process_event_level_attribute(
    ...
    event_misp_type = field_def.get('type_hint') or _infer_misp_type_from_name(attr_name) or misp_type
    attr = {
        "uuid": str(uuid.uuid4()),
...
def _should_be_ids(misp_type: str) -> bool:
    ...
    ids_types = {
        'md5', 'sha1', 'sha256', 'sha512', 'ssdeep', 'imphash', 'tlsh',
        'ip-src', 'ip-dst', 'domain', 'hostname', 'url', 'filename'
    }
...
def _parse_dsl(field_def: str) -> Dict:
    ...
    block_if_re = re.match(r"^\{\{#if\s+(.+?)\}\}(.*?)(?:\{\{#else\}\}(.*?))?\{\{/if\}\}\s*$", text, flags=re.DOTALL)
    if block_if_re:
        result['handler'] = 'if'
        result['handler_path'] = block_if_re.group(1).strip()
        result['true_block'] = (block_if_re.group(2) or '').strip()
        result['false_block'] = (block_if_re.group(3) or '').strip() if block_if_re.group(3) is not None else None
        return result
    ...
    block_foreach_re = re.match(r"^\{\{#foreach\s+(.+?)\}\}(.*?)\{\{/foreach\}\}\s*$", text, flags=re.DOTALL)
    if block_foreach_re:
...
def _build_attributes_from_def(
    ...
    for field_name, field_def in obj_def.items():
        ...
        is_tag = field_name.startswith(":") or (":" in field_name and not field_name.startswith("obj:"))
        ...
        if handler_name == "ref":
            ...
            if filter_match:
                ...
                if '=' in filter_expr:
                    ...
                    foreach_filter = (filter_field.strip(), [v.strip() for v in filter_vals.split(',')])
        ...
        is_foreach_aggregate = handler_name == 'foreach' and not is_multiple_mode and resolved_values
        ...
        misp_type = field_def.get('type_hint') or _infer_misp_type_from_name(actual_field_name) or _determine_misp_type(value)
        ...
        if promote_to_event and value is not None:
            # Priority: type_hint from DSL > inferred from field name > determined from value
            event_misp_type = field_def.get('type_hint') or _infer_misp_type_from_name(actual_field_name) or misp_type
            event_attr = {
                "uuid": str(uuid.uuid4()),
                "type": event_misp_type,
                "value": value,
...
def _process_array_objects(
    ...
    for obj_name, obj_def in ioc_mappings.items():
        ...
        for item in items:
            ...
            for field_name, field_def in obj_def.items():
                # Skip directives
                if field_name.startswith("obj:") or field_name.startswith("_") or field_name.startswith("#"):
                    continue
...
def _format_statistics(stats: Any) -> str:
    ...
    if isinstance(stats, dict):
        ...
        if stats:
            return f"AV results: {', '.join(str(item) for item in stats[:10])}" + ("..." if len(stats) > 10 else "")
        return ""
...
def _build_unified_summary(data: Dict, api_context: Optional[Dict[str, Any]] = None) -> str:
    ...
    if sample:
        ...
        add_unique("Risk Score", sample.get("riskscore"), lambda x: x is not None, lambda x: f"{x}/10")
        ...
        add_unique("Extracted Files", sample.get("extracted_file_count"), lambda x: x is not None and x > 0)
    ...
    if av.get("scanner_count"):
        match, count, pct = av.get("scanner_match", 0), av["scanner_count"], av.get("scanner_percent", 0)
    ...
    if tpr:
        ...
        if total:
            status = f"{mal}/{total} vendors flagged malicious" if mal else f"Clean ({total} vendors checked)"
            add_unique("Reputation", status)

    # Top Threats
    threats = [t.get("threat_name") for t in data.get("top_threats", [])[:5] if t.get("threat_name")]
    if threats:
    ...
    if not sample:
        ...
        add_unique("Risk Score", data.get("riskscore"), lambda x: x is not None, lambda x: f"{x}/10")
