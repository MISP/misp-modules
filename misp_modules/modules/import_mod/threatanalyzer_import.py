...
moduleinfo = {
    ...
    "requirements": [],
    "features": (
        "The module imports MISP Attributes from a ThreatAnalyzer format file. This file can be either ZIP, or JSON"
    ...
    if data[: len(zip_starts)].decode() == zip_starts:
        with zipfile.ZipFile(io.BytesIO(data), "r") as zf:
            ...
            for zip_file_name in zf.namelist():  # Get all files in the zip file
                ...
                if (
                    re.match(r"Analysis/proc_\d+/modified_files/.+\.", zip_file_name)
                    and "mapping.log" not in zip_file_name
                ):
                        ...
                        results.append(
                            {
                                ...
                                "data": base64.b64encode(file_data).decode(),
                                "type": "malware-sample",
                                "categories": ["Payload delivery", "Artifacts dropped"],
                                "to_ids": True,
                                "comment": "",
                            }
                        )
...
def process_analysis_json(analysis_json):
    if (
        ...
        and "processes" in analysis_json["analysis"]
        and "process" in analysis_json["analysis"]["processes"]
    ):
        ...
        for process in analysis_json["analysis"]["processes"]["process"]:
            ...
            if "connection_section" in process and "connection" in process["connection_section"]:
                ...
                for connection_section_connection in process["connection_section"]["connection"]:
                    ...
                    if "name_to_ip" in connection_section_connection:  # TA 6.1 data format
                        connection_section_connection["@remote_ip"] = connection_section_connection["name_to_ip"][
                            "@result_addresses"
                        ]
                        connection_section_connection["@remote_hostname"] = connection_section_connection["name_to_ip"][
                            "@request_name"
                        ]

                    connection_section_connection["@remote_ip"] = cleanup_ip(
                        connection_section_connection["@remote_ip"]
                    )
                    connection_section_connection["@remote_hostname"] = cleanup_hostname(
                        connection_section_connection["@remote_hostname"]
                    )
                    if (
                        connection_section_connection["@remote_ip"]
                        and connection_section_connection["@remote_hostname"]
                    ):
                        val = "{}|{}".format(
                            connection_section_connection["@remote_hostname"],
                            connection_section_connection["@remote_ip"],
                        ...
                        yield (
                            {
                                "values": connection_section_connection["@remote_ip"],
                                "type": "ip-dst",
                                "to_ids": True,
                                "comment": "",
                            }
                        ...
                        yield (
                            {
                                "values": connection_section_connection["@remote_hostname"],
                                "type": "hostname",
                                "to_ids": True,
                                "comment": "",
                            }
            ...
            if "networkoperation_section" in process and "dns_request_by_addr" in process["networkoperation_section"]:
                for networkoperation_section_dns_request_by_addr in process["networkoperation_section"][
                    "dns_request_by_addr"
                ]:
            ...
            if "networkoperation_section" in process and "dns_request_by_name" in process["networkoperation_section"]:
                for networkoperation_section_dns_request_by_name in process["networkoperation_section"][
                    "dns_request_by_name"
                ]:
                    networkoperation_section_dns_request_by_name["@request_name"] = cleanup_hostname(
                        networkoperation_section_dns_request_by_name["@request_name"].rstrip(".")
                    )
                    networkoperation_section_dns_request_by_name["@result_addresses"] = cleanup_ip(
                        networkoperation_section_dns_request_by_name["@result_addresses"]
                    )
                    if (
                        networkoperation_section_dns_request_by_name["@request_name"]
                        and networkoperation_section_dns_request_by_name["@result_addresses"]
                    ):
                        val = "{}|{}".format(
                            networkoperation_section_dns_request_by_name["@request_name"],
                            networkoperation_section_dns_request_by_name["@result_addresses"],
                        ...
                        yield (
                            {
                                "values": networkoperation_section_dns_request_by_name["@request_name"],
                                "type": "hostname",
                                "to_ids": True,
                                "comment": "",
                            }
                        ...
                        yield (
                            {
                                "values": networkoperation_section_dns_request_by_name["@result_addresses"],
                                "type": "ip-dst",
                                "to_ids": True,
                                "comment": "",
                            }
            ...
            if "networkpacket_section" in process and "connect_to_computer" in process["networkpacket_section"]:
                for networkpacket_section_connect_to_computer in process["networkpacket_section"][
                    "connect_to_computer"
                ]:
                    networkpacket_section_connect_to_computer["@remote_hostname"] = cleanup_hostname(
                        networkpacket_section_connect_to_computer["@remote_hostname"]
                    )
                    networkpacket_section_connect_to_computer["@remote_ip"] = cleanup_ip(
                        networkpacket_section_connect_to_computer["@remote_ip"]
                    )
                    if (
                        networkpacket_section_connect_to_computer["@remote_hostname"]
                        and networkpacket_section_connect_to_computer["@remote_ip"]
                    ):
                ...
                for stored_created_file in process["stored_files"]["stored_created_file"]:
                    ...
                    if stored_created_file["@filename"]:
                        if stored_created_file["@filesize"] != "0":
                            val = "{}|{}".format(
                                stored_created_file["@filename"],
                                stored_created_file["@md5"],
                            )
                            ...
                            yield (
                                {
                                    "values": stored_created_file["@filename"],
                                    "type": "filename",
                                    "to_ids": True,
                                    "categories": [
                                        "Artifacts dropped",
                ...
                for stored_modified_file in process["stored_files"]["stored_modified_file"]:
                    ...
                    if stored_modified_file["@filename"]:
                        if stored_modified_file["@filesize"] != "0":
                            val = "{}|{}".format(
                                stored_modified_file["@filename"],
                                stored_modified_file["@md5"],
                            )
                            ...
                            yield (
                                {
                                    "values": stored_modified_file["@filename"],
                                    "type": "filename",
                                    "to_ids": True,
                                    "categories": [
                                        "Artifacts dropped",
...
def cleanup_ip(item):
    ...
    noise_substrings = {"224.0.0.", "127.0.0.", "8.8.8.8", "8.8.4.4", "0.0.0.0", "NONE"}
...
def cleanup_hostname(item):
    noise_substrings = {"wpad", "teredo.ipv6.microsoft.com", "WIN7SP1-x64-UNP"}
