...
class TestExports(unittest.TestCase):
    ...
    def test_introspection(self):
        ...
        try:
            ...
            export_mod_path = os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "..", "misp_modules", "modules", "export_mod"
            )
            module_files = [
                file[:-3]
                for file in os.listdir(export_mod_path)
                if file.endswith(".py")
                if file not in ["__init__.py", "testexport.py"]
            ]
            missing = []
    ...
    def test_threat_connect_export(self):
        ...
        query = {"module": "threat_connect_export", "data": [self.event], "config": {"Default_Source": test_source}}
    ...
    def test_yara_export(self):
        ...
        response = self.misp_modules_post(query)
        expected_result = (
            'rule MISP_e625_MetadataExample\n{\n    meta:\n        my_identifier_1 = "Some string data"\n       '
